...
const paymentController = async (req, res, _next) => {
    ...
    const response = await axios_1.default.get("http://localhost:3001/carts/me", {
    ...
    const user = await jwt.verify(refreshTokens, secret_1.refresh_key);
    if (!user) {
    ...
    sslcz.init(data)
        ...
        console.log(info);
        const gg = await axios_1.default.post("http://localhost:3001/payment/create", info);
        const pp = gg.data;
